...
|---|---|---|
| `sets.js` | `[Set-Code, Name]` | the-fab-cube `set.csv` |
...
Die Anwendung lädt `set.csv`, `card.csv` und `card-printing.csv` beim Start zusätzlich online
und verwendet sie, wenn das gelingt. Die mitgelieferten Dateien sind der Rückfall ohne Internet.
Beide Wege nutzen dieselbe Umwandlung (`app/reference-transform.js`).

## Erneuern

1. Die drei Quelldateien aus `csvs/english/` des Datensatzes in einen Ordner **außerhalb** des
   Repositorys laden.
